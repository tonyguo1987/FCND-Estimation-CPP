...
V3F QuadControl::RollPitchControl(V3F accelCmd, Quaternion<float> attitude, float collThrustCmd)
{
	...
	if (collThrustCmd > 0.0) {
		float a = -collThrustCmd / mass;
		float target_R13 = CONSTRAIN(accelCmd.x / a, -maxTiltAngle, maxTiltAngle);
		float target_R23 = CONSTRAIN(accelCmd.y / a, -maxTiltAngle, maxTiltAngle);
...
float QuadControl::YawControl(float yawCmd, float yaw)
{
	...
	yaw_err = fmodf(yaw_err, F_PI * 2.f);
