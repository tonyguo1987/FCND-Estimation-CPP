...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
	...
	// Create quaternion using estimated roll pitch and yaw
	Quaternion <float> attitude = Quaternion <float>().FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
	// Integrate gyro rate in current attitude
	attitude.IntegrateBodyRate(gyro, dtIMU);
	// Get integrated attitude
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
	...
	////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
	// Convert accel from body frame to inertial frame
	V3F acc_inert = attitude.Rotate_BtoI(accel);
	predictedState(0) += curState(3) * dt;
	predictedState(1) += curState(4) * dt;
	predictedState(2) += curState(5) * dt;
	predictedState(3) += acc_inert.x * dt;
	predictedState(4) += acc_inert.y * dt;
	predictedState(5) += (acc_inert.z - CONST_GRAVITY) * dt;
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
	...
	RbgPrime(2, 0) = RbgPrime(2, 1) = RbgPrime(2, 2) = 0.f;
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
	...
	hPrime(0, 6) = 1.;
	zFromX(0) = ekfState(6);
	// normalize the difference between measured and estimated yaw
